...
%%%%%%%%%%%%%%%%%%
% Should we start with place-value / exploding dots? 
...
%\begin{example}
%	Draw a number line diagram to represent \(15-8=7\).
%\end{example}
\begin{exercise}
	Draw a number line diagram for each of the operations below. 
	\[ 38 - 11 = 27 \]
...
\begin{exercise}
Justify each of the above cases for multiplication.  %The same justifications will hold for division.  	
...
\vspace*{25mm} 
% 10 x 10 Grid for Exercise~\ref{ex:gcdgrid}
\newcommand*{\xMin}{0}%
\newcommand*{\xMax}{10}%
\newcommand*{\yMin}{0}%
\newcommand*{\yMax}{10}%
\hspace{-15mm}\begin{tikzpicture}[scale=1.60]
    \foreach \i in {\xMin,...,\xMax} {
        \draw [very thin,gray] (\i,\yMin) -- (\i,\yMax)  node [below] at (\i-0.5,\yMin) {$\i$};
    }
    \foreach \i in {\yMin,...,\yMax} {
        \draw [very thin,gray] (\xMin,\i) -- (\xMax,\i) node [left] at (\xMin,\i-0.5) {$\i$};
    }
\end{tikzpicture}
...
% Does this go here, or does is it really part of the next section, ``Evaluating Expressions''?
